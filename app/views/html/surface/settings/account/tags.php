  ...
  if(isset($_POST) && !empty($_POST)){
    ...
    if(isset($_POST['portal']) && !empty($_POST['portal'])){
        ...
        if($_POST['portalsubmit'] == 'level_7'){  
            $geturl = $webAPI."/dcc/api/tag/deletetag/".$_POST['tagId']."?".$vccommQueryParam;
            $postData = "";
            ...
            $geturl = $webAPI."/dcc/api/user/setstatus/".$_POST['tagId']."?".$vccommQueryParam;
            ...
            $geturl = $webAPI."/dcc/api/tag/updatetag"."?".$vccommQueryParam;
                        $postData = json_decode($_POST['data']);
            ...
            $geturl = $webAPI."/dcc/api/tag/addtag"."?".$vccommQueryParam;
                        $postData = json_decode($_POST['data']);
            ...
            $posturl = $webAPI."/dcc/api/tag/gettag/".$_POST['tagId']."?".$vccommQueryParam;
            $result = $curlPHPhelp->curl_GET_Auth($posturl,"");
            $final_result = $result;
           
            echo json_encode($final_result);exit;
        }else if($_POST['portalsubmit'] == 'level_2'){      
            $posturl = $webAPI."/dcc/api/tag/getinitadd?".$vccommQueryParam;
            $result = $curlPHPhelp->curl_GET_Auth($posturl,"");
            $final_result = $result;
            ...
            $posturl = $webAPI."/dcc/api/tag/getalltags?".$vccommQueryParam."&start=".$start."&length=".$length."".$searchKeywords."".commonTableSortingParam($_POST);
            $result = $curlPHPhelp->curl_GET_Auth($posturl,"");
            $final_result = $result;
